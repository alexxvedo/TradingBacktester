...
export default function PositionCreator({
  ...
  const closePosition = (key) => async () => {
    ...
    try {
      const res = await fetch(`/api/operations/${key}`, {
        ...
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${session.user.token}`, // Añadir token para autenticación
        },
        body: JSON.stringify({
  ...
  return (
    <div className="flex flex-col items-center max-w-[30%] h-full justify-center py-5 gap-4">
